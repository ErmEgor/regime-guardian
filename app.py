...
async def cq_help_functionality(callback: CallbackQuery):
    ...
    try:
        ...
        help_text = (
            ...
            "В боте есть утренний опрос, проходя который, можно определить для себя лимит экранного времени и главные активности, которые вы выполняете в течении дня.\n\n"
            "Также присутствует вечерняя сводка, в которой можно посмотреть итоги по экранному времени и главным активностям за день. После этого необходимо отметить выполнение или невыполнение привычек и целей, если таковые имеются. И в конце отметки необходимо лично для себя ответить на тройку вопросов, помогающих в анализе и улучшении своих дней.\n\n"
            "Напоминание за час до сводки помогает не забыть отметиться."
        )
        await callback.message.edit_text(
            help_text,
            reply_markup=keyboards.get_help_menu_keyboard()
...
async def cq_help_activities(callback: CallbackQuery):
    ...
    try:
        help_text = (
            "🏃 <b>Активности</b>\n\n\n"
            "Есть 2 типа активностей.\n\n"
            "1. Главные или предустановленные. Они выбираются в утреннем опросе в качестве запланированных на день.\n\n"
