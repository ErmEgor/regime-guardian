...
async def cq_help_general(callback: CallbackQuery):
    ...
    try:
        help_text = (
            "📖 <b>Общее</b>\n\n"
            "Данный бот создал Егор Ермилов. Он служит помощником с становлении лучшей версией себя. "
            "Я расчитываю на то, что вы будете не просто слепо выполнять опросы и задачи, но и работать над собой, анализировать ваши дни и улучшать их.\n"
            "Для начала вам нужно пройти утренний опрос через /morning, после чего записывать ваши активности и работать над собой. "
...
async def cq_help_activities(callback: CallbackQuery):
    ...
    try:
        help_text = (
            "🏃 <b>Активности</b>\n\n"
            "Есть 2 типа активностей. Главные или предустановленные. Они выбираются в утреннем опросе в качестве запланированных на день. "
            "Второй тип это свободные или те, которые вы сами можете записать. Они делятся на полезные и не полезные. "
...
async def cq_help_tips(callback: CallbackQuery):
    ...
    try:
        help_text = (
            ...
            "Если вы реально хотите изменить себя , вы должны применить их в жизни, а не просто прочесть и забыть."
        )
        await callback.message.edit_text(
            help_text,
            reply_markup=keyboards.get_help_menu_keyboard()
